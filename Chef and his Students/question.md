...
Chef is instructor of the famous course "Introduction to Algorithms" in a famous university. There are `n` students in his class. There is not enough space in the classroom, so students sit in a long hallway in a linear fashion.
...
For example, consider a sample configuration of students - `*><*`. Here students numbered `1` and `4` are busy studying, while the student `2` and `3` are talking to each other. In this example, `><><`, student `1` and `2` are talking to each other, and `3` and `4` are also talking to each other.

You are guaranteed that the given input is a valid configuration.

When the students see their teacher coming, those who were talking get afraid and immediately turn around, i.e. students talking to left have now turned to the right, and the one talking to right have turned to the left. When Chef sees two students facing each other, he will assume that they were talking. A student who is busy studying will continue doing so.
