...
Chef has `N` small boxes arranged on a line from `1` to `N`. For each valid `i`, the weight of the `i`-th box is `Wᵢ`. Chef wants to bring them to his home, which is at the position `0`.

He can hold any number of boxes at the same time; however, the total weight of the boxes he's holding must not exceed `K` at any time, and he can only pick the `i`-th box if all the boxes between Chef's home and the `i`-th box have been either moved or picked up in this trip.
...
- The second line contains `N` space-separated integers `W₁, W₂, …, Wₙ`.
